...
- [九、大模型训练流程：预训练、指令微调、偏好优化](#九大模型训练流程预训练指令微调偏好优化)
...
## 九、大模型训练流程：预训练、指令微调、偏好优化

理解 LLM，不只是理解结构，还要理解它是怎么从会预测 token走向会听指令、会符合偏好的。

### 1. 预训练

预训练通常在大规模语料上进行。  
对于 decoder-only 模型，典型目标是下一 token 预测。
...
> 不全量更新整个模型，而是只训练一小部分附加参数，以更低成本实现微调。

LoRA 是最经典的 PEFT 方法之一。
...
> [!TIP]
> 对初学者来说，第一遍学微调，优先学会 LoRA，比直接碰全量微调更实际。

### 4. DPO 与 PPO

当模型已经具备基本指令能力后，还会进一步追求更符合人类偏好。
